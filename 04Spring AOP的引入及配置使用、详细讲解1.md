...
**IOC是bean的容器，往里面存放bean对象，方便对象的获取，之后bean对象不需要自己创建了**
...
public class LogUtil {
    ...
    public static void start(Object ... objects){ //... 可变参数
...
public class LogUtil {
    ...
    public static void start(Method method,Object ... objects){ //... 可变参数
...
Proxy类（java.lang.reflect.Proxy），有newProxyInstance()方法
...
public class CalculatorProxy {
     ...
     * @return
     */
    public static Calculator getProxy(final Calculator calculator){//因为在匿名内部类要调用该参数calculator,所以外部的参数calculator必须是final的
        ...
        //方法执行器，执行被代理对象的目标方法【用来执行被代理类需要执行的方法】
...
代理对象和被代理对象有什么区别？被代理对象实现目标接口，继承Proxy类

JDK动态代理必须要有接口，如果没有接口，不能使用，这种方式用jdk提供的reflect包下的类，但是在生产环境中我不能保证每个类都有实现的接口，所以有第二种方式cglib；cglib在实现的时候有没有接口都无所谓。

**在spring中使用了两种动态代理的方式,一种是jdk提供的，一种是CGLIB**

动态代理和静态代理的区别：动态代理对象每次创建不固定(@proxy4)，是在代码执行过程中动态生成的；而静态代理的代理对象每次都是固定的
...
JDK的动态代理，底层用的是反射（在InvocationHandler的invoke方法中调用了method.invoke(目标类)）
...
![401](D834AEFD16394EB085E7623EDA722350)

- 切面（Aspect）: 指关注点模块化，这个关注点可能会横切多个对象。事务管理是企业级Java应用中有关横切关注点的例子。 在Spring AOP中，切面可以使用通用类基于模式的方式（schema-based approach）或者在普通类中以`@Aspect`注解（@AspectJ 注解方式）来实现。【就是切面类，也就是LogUtil】
- 连接点（Join point）: 在程序执行过程中某个特定的点，例如某个方法调用的时间点或者处理异常的时间点。在Spring AOP中，一个连接点总是代表一个方法的执行。【calculatorProxy类中调用LogUtil方法的地方就是连接点】
- 通知（Advice）: 在切面的某个特定的连接点上执行的动作。通知有多种类型，包括“around”, “before” and “after”等等。通知的类型将在后面的章节进行讨论。 许多AOP框架，包括Spring在内，都是以拦截器做通知模型的，并维护着一个以连接点为中心的拦截器链。
- 切点（Pointcut）: 匹配连接点的断言。通知和切点表达式相关联，并在满足这个切点的连接点上运行（例如，当执行某个特定名称的方法时）。切点表达式如何和连接点匹配是AOP的核心：Spring默认使用AspectJ切点语义。【实际切入的地方就是切入点】
...
- 目标对象（Target object）: 被一个或者多个切面所通知的对象。也被称作被通知（advised）对象。既然Spring AOP是通过运行时代理实现的，那么这个对象永远是一个被代理（proxied）的对象。【myCalculator】
- AOP代理（AOP proxy）:AOP框架创建的对象，用来实现切面契约（aspect contract）（包括通知方法执行等功能）。在Spring中，AOP代理可以是JDK动态代理或CGLIB代理。【$proxy】
- 织入（Weaving）: 把切面连接到其它的应用程序类型或者对象上，并创建一个被被通知的对象的过程。这个过程可以在编译时（例如使用AspectJ编译器）、类加载时或运行时中完成。 Spring和其他纯Java AOP框架一样，**是在运行时完成织入的**。


**概念很重要，之后在编写代码的时候你才能知道在什么地方需要编写什么代码**
...
```xml
        ...
        <!-- https://mvnrepository.com/artifact/org.aspectj/aspectjweaver  提供更强功能的织入-->
        <dependency>
...
##### 3、测试
  ...
  public class MyTest {
      public static void main(String[] args){
          ...
          //Calculator calculator = context.getBean( "calculator",Calculator.class);//这个代码是错误的，因为接口不能实例化，所以按照name来查找对象的时候找不到；根据类型会找到对应的子类实现
          bean.add(1,1);
          System.out.println(calculator.getClass());
      }
}

执行结果：注意执行顺序有问题，后续会讲执行顺序
方法开始执行：参数是
方法执行结束。。。。。over
方法执行结束，结果是：
...
​		可以通过cglib的方式来创建代理对象，此时不需要实现任何接口，代理对象是class com.mashibing.inter.MyCalculator$$EnhancerBySpringCGLIB$$1f93b605类型

​		**综上所述：在spring容器中，如果有接口，那么会使用jdk自带的动态代理，如果没有接口，那么会使用cglib的动态代理。动态代理的实现原理，后续会详细讲.**

==JDK的动态代理和CGLIB代理的效率问题：早期的JDK的动态代理是比较慢的，而JDK不断的更新，现在效率也挺快的；而CGLIB早期确实挺快；现在两个的效率差不多==

##### 注意：

###### 1、切入点表达式

最精确的匹配方式：public Integer com.mashibing.service.MyCalculator.add(Integer,Integer)
...
​		4、权限位置(访问修饰符)不能使用*，如果想表示全部权限（访问修饰符），那么不写即可

​				execution( * com.mashibing.inter.MyCalculator.*(int,*))

.       5.返回值可以使用*来代替

.       execution(public * com.mashibing.inter.MyCalculator.*(int,*))
...
​		在写表达式的时候，可以有N多种写法，但是有一种最偷懒和最精确的方式：execution(* com.mashibing..MyCalculator*.*(..))
...
execution(* com.*(..))这个表达式不行，应该是execution(* com.*.*(..))【因为形参列表的前面必须跟的是方法名称，第一个是包名】

==使用通配符的时候不是越简洁越好，更多的是要选择符合要求或符合规则的匹配方式，此时就要求在定义标识符的时候必须要遵循项目规范==

​				最精确的方式：execution( public int com.mashibing.inter.MyCalculator.add(int,int))

​		除此之外，在表达式中还支持逻辑运算： &&、||、！的方式

==注意返回值类型，如果返回值类型不匹配也无法匹配==

​				&&：两个表达式同时满足

​				execution( public Integer com.mashibing.inter.MyCalculator.*(..)) && execution(* *.*(int,int) )

​				||：任意满足一个表达式即可

​				execution( public Integer com.mashibing.inter.MyCalculator.*(..)) || execution(* *.*(int,int) )

​				！：取反，除了这种情况的其他都满足；只要不是这个位置都可以进行切入

!execution(public Integer com.mashibing.service.MyCalculator.add(Integer,Integer))

###### 表达式的另外一种写法：@annotation

![402](B5774A543CAC4220867B4EC1FE525F6E)
...
==不容易理解的原因是：最后才是"return result""所有@AfterReturnning最后执行==
